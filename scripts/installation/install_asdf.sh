...
# check if asdf exist if not install for kube-score, version 0.18.0
...
# check if asdf exists oor install it
if ! command -v asdf >/dev/null 2>&1; then
    echo "==> ASDF n est pas installé."
    echo "==> Installation de la dernière version stable de asdf ..."
    ...
    echo "==> ASDF est maintenant installé en version: $CURRENT_ASDF_VERSION"

# check asdf version and upgrade if necessary
    if [ -z "$CURRENT_ASDF_VERSION" ]; then
        ...
        echo "==> Mise à jour de la dernière version stable de asdf ..."
        wget -q https://github.com/asdf-vm/asdf/releases/download/v0.18.0/asdf-v0.18.0-linux-amd64.tar.gz
        tar -xzf asdf-v0.18.0-linux-amd64.tar.gz -C "$HOME/.local/bin"
        chmod +x "$HOME/.local/bin/asdf"
        rm -f asdf-v0.18.0-linux-amd64.tar.gz
