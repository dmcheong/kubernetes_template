...
kubeseal --controller-name=sealed-secrets-controller --controller-namespace=kube-system --format=yaml < "$SCRIPT_DIR/kong-gateway-secret.yaml" > "$SCRIPT_DIR/kong-gateway-sealed-secret.yaml"
...
DOCKER_CONFIG=$HOME/.docker-helm helm upgrade --install kong-postgresql bitnami/postgresql -n kong -f "$SCRIPT_DIR/postgres-values.yml" --set auth.postgresPassword="$POSTGRES_PASSWORD"
...
helm upgrade --install kong kong/ingress -n kong -f "$SCRIPT_DIR/kong-values.yml"

# check all ressource about kong
echo "==> Verification et affichage de toutes les ressources dans le namespace kong:"
